$(document).ready(function() {
  ...
  var results = [];
  var apiShortCut = [];
  apiShortCut.push(espnApi, natGeoApi, economistApi, nextWebApi, vergeApi, buisInsdrApi);
    ...
    $(cls).click(function() {
      $.ajax({
        ...
        success: function(results) {
          ...
          for (var i = 0; i < 6; i++) {
            var list = '<div><div class="headline"><h2>' + results.articles[i].title + '</h2></div>';
            list += '<div class="slide__copy">ESPM:' + results.articles[i].description + '</div>';
            list += '<div><a class="learnMore" href="' + results.articles[i].url + '" target ="_blank">Learn more &#xbb;</a></div>';
            list += '</div>';
 ...
 $('.slider').slick('slickRemove');
    ...
    $.ajax({
     ...
     $(".slider").children()[i].child("img").attr("src", response.articles[j].urlToImage)
